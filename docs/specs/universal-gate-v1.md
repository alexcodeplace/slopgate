...
Status: authoritative implementation contract. Contract changes require a recorded architecture decision and owner-authorized review. The owner explicitly authorized autonomous review and merge (ADR 0004); a second human reviewer is not a prerequisite. Implementation agents MUST NOT change architecture, weaken correctness tests, or bypass required CI merely to obtain a passing result.
...
SG-GOV-002: Protect spec, architecture policy/guard, workflows, manifests/lockfile, baseline/suppression policy, rules and CODEOWNERS. Hosting must require pull requests, up-to-date named passing checks bound to their expected GitHub App, resolved conversations, and no force pushes/deletion, with restrictions applying to administrators. The owner-authorized autonomous reviewer may review and merge its own implementation after verified checks; zero independent approvals and no mandatory code-owner/latest-pusher approval are intentional. CODEOWNERS is a routing/audit aid, not a second-human dependency. Required checks MUST NOT be bypassed. Hosting changes require explicit owner authorization; this delivery includes the requested activation. Trusted-base metadata-only drift checking must never execute candidate code or load candidate policy with privileged permissions.

SG-GOV-003: Spec changes require an ADR with compatibility/performance impact, acceptance evidence and recorded review authorization, not a checksum edited to excuse a failing implementation. Owner-authorized self-review is permitted and must be identified honestly; do not manufacture an independent approval. Administrators controlling code/checks/protections can change repository safeguards, and shared GitHub Actions identity is not an isolated adversarial trust boundary. Disclose those limits without inventing a requirement for the owner to recruit another reviewer. Report feature-branch implementation separately from actual hosted activation. Bootstrap may temporarily require the seven existing CI checks until the trusted workflow reaches main; immediately add the trusted status after merge and verify it with a disposable negative/positive PR. Never mark hosted enforcement active before verification.
