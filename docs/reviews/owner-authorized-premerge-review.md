...
The live GitHub API rejected a protection payload containing both `contexts` and `checks` because they are alternative selectors. The stored template now uses only App-bound `checks`, with a regression asserting it is directly applicable. The verifier derives the required names from those explicit records. No branch setting was changed by the rejected request. The corrected seven-check bootstrap payload was then successfully applied, including administrator enforcement and zero required approvals.
