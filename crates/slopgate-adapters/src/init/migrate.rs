...
"#;
    let abs_string = abs.to_string_lossy();
    let output = slopgate_core::process::run_tool(
        Path::new("node"),
        &["-e", SCRIPT, &abs_string],
        abs.parent(),
        Some(10_000),
...
pub fn migrate_legacy_config(target_dir: &Path) -> MigrateOutcome {
    ...
    let mut export = match read_legacy_export(&legacy) {
        Ok(v) => v,
        Err(reason) => {
    ...
    // Preserve relative policy assets when moving from the pre-rename folder.
    // Do not silently redirect existing rules to a new empty directory.
    if legacy.parent() != toml_path.parent() {
        for key in ["astRules", "suppressions", "fixtures"] {
            if let Some(value) = export.get(key).and_then(Value::as_str).map(str::to_string) {
                if !Path::new(&value).is_absolute() {
                    let old_relative = legacy
                        .parent()
                        .and_then(|parent| parent.strip_prefix(target_dir).ok());
                    if let Some(old_relative) = old_relative {
                        export[key] = Value::String(format!(
                            "../{}/{}",
    ...
    if !dropped_rules.is_empty() {
        return MigrateOutcome::Failed { from: legacy, reason: format!("migration would drop required legacy rule packs: {}; re-author them before migrating", dropped_rules.join(", ")) };
    }
    let config_dir = toml_path.parent().expect("config path has parent");
    if export.get("astRules").is_none() {
        if let Err(error) = std::fs::create_dir_all(config_dir.join("rules/ast")) {
            return MigrateOutcome::Failed {
                from: legacy,
                reason: format!("create default rules directory: {error}"),
            };
        }
    }
    if let Err(e) = slopgate_core::config::resolve_config_text_at(&toml_src, config_dir, target_dir)
    {
        return MigrateOutcome::Failed {
            from: legacy,
