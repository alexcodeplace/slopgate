...
use slopgate_core::error::SlopError;
use slopgate_core::install::agent_hooks::{
    home_dir, install_agent_hooks as install_detected_agent_hooks,
};
use slopgate_core::install::hooks::{install_pre_commit_hook, HookInstallAction};
use slopgate_core::install::skills::{default_skills_dest_in, install_skills};
use std::fs;
use std::io::Write;
use std::path::Path;
...
pub use slopgate_core::assets::engine_root;
...
fn run_init_inner(
    ...
    if let MigrateOutcome::Failed { from, reason } = &migrate_outcome {
        return Err(SlopError::Io(format!(
            "refusing to replace failed legacy policy {} with a fresh scaffold: {reason}",
            from.display()
        )));
    }
