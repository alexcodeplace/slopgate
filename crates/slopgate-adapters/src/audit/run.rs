...
use slopgate_core::enumerate::{list_source_files, EnumerateCtx, EnumerateMode};
use slopgate_core::gate::snapshot_violations;
use slopgate_core::ratchet::load_baseline;
use slopgate_core::stats::query::{aggregate, Row};
use slopgate_core::stats::store::{project_stats_path, read_rows};
use slopgate_core::suppressions::{load_suppressions, prune_stale};
...
fn gate_effectiveness_lines(stats: &slopgate_core::stats::query::AggregateResult) -> Vec<String> {
    if stats.total == 0 {
        return Vec::new();
    }
    ...
    let outer: Result<(), String> = (|| {
        ...
        // Ratchet progress + burn-down
        {
            ...
            if bl.missing {
                ...
                match snapshot_violations(config, crate::checkers::index::CHECKERS) {
                    Ok(findings) => sections.push(AuditSection {
                        title: "Ratchet progress + burn-down".into(),
                        lines: ratchet_lines(bl.entries.len(), findings.len(), &burndown),
                    }),
                    Err(errors) => notices.push(format!(
                        "ratchet progress incomplete: {}",
                        errors.join("; ")
                    )),
                }
            }
        ...
        // Gate effectiveness (stats.jsonl)
        {
            ...
            if raw.is_empty() {
                ...
                let stats = aggregate(&rows, None, None).unwrap_or(
                    slopgate_core::stats::query::AggregateResult {
                        total: 0,
                        by: "rule".into(),
                        last_seen: None,
                        first_seen: None,
...
mod tests {
    use super::*;
    use serde_json::json;
    use slopgate_core::config::resolve_config_str;
    use std::fs;
