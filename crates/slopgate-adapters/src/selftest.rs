...
use slopgate_core::ast_engine::{run_ast_grep_scan, AstGrepScanOpts};
use slopgate_core::config::ResolvedConfig;
use slopgate_core::regex_engine::evaluate_text_pattern;
use std::fs;
...
fn regex_test(pattern: &str, flags: &str, text: &str) -> Result<bool, String> {
    evaluate_text_pattern(pattern, flags, text)
}
...
pub fn run_self_test(config: &ResolvedConfig) -> i32 {
    let mut failed = 0i32;
    let mut required_project_rules: std::collections::BTreeSet<String> =
        config.project_rule_ids.iter().cloned().collect();
    ...
    for p in &config.patterns {
        let Some(canary) = &p.canary else {
            ...
            if config.project_rule_ids.contains(&p.id) {
                failed += 1;
            }
            continue;
        };
        if config.project_rule_ids.contains(&p.id)
            && p.negative_canary.as_ref().is_none_or(Vec::is_empty)
        {
            eprint(&format!(
                "FAIL {}: project rule needs a negative canary",
                p.id
            ));
            failed += 1;
        }
    ...
    if config.ast_enabled && (!ast.available || !ast.errors.is_empty()) {
        eprint(&format!(
            "FAIL required AST self-test incomplete: {}",
            ast.errors.join("; ")
        ));
        failed += 1;
    } else if config.ast_enabled
        && !ast.violations.iter().any(|v| v.id == "slopgate-canary")
        && config.project_rule_ids.is_empty()
        && config
            .fixtures_dirs
            .iter()
            .all(|path| path.contains("rules/baseline/fixtures"))
    {
        for e in &ast.errors {
        ...
        for dir in project_ast_dirs {
            ...
            for entry in entries.flatten() {
                ...
                if config
                    .project_ast_dirs
                    .iter()
                    .any(|directory| Path::new(directory) == Path::new(dir))
                {
                    required_project_rules.insert(id.clone());
                }
    ...
    if !required_project_rules.is_empty() {
        let contracts: Vec<_> = config
            .fixtures_dirs
            .iter()
            .map(|directory| Path::new(directory).join("expectations.json"))
            .filter(|path| path.is_file())
            .collect();
        if contracts.len() != 1 {
            eprint("FAIL project policy requires exactly one expectations.json fixture contract");
            failed += 1;
        } else {
            match slopgate_core::fixture_contract::verify(
                config,
                &contracts[0],
                &required_project_rules,
                crate::checkers::index::CHECKERS,
            ) {
                Ok(count) => eprint(&format!("OK {count} exact project fixture cases")),
                Err(errors) => {
                    failed += 1;
                    for error in errors {
                        eprint(&format!("FAIL {error}"));
                    }
                }
            }
        }
    }
    // Parser conformance belongs to shipped checker implementations, not files
    // a consumer project must copy into its own fixture directory.
    let mut engine_fixture_config = config.clone();
    engine_fixture_config.fixtures_dirs = vec![crate::init::run::engine_root()
        .join("rules/baseline/fixtures")
        .to_string_lossy()
        .into_owned()];
    let fix_dir = checker_fixtures_dir(&engine_fixture_config);
...
mod tests {
    ...
    use slopgate_core::config::GateAllow;
    use slopgate_core::rules::packs::{baseline_packs, ux_packs, Pattern};
    ...
    fn build_project_pack_selftest_config(dir: &Path, proj_json: &str) -> ResolvedConfig {
        ...
        write_temp_file(
            &fixtures.join("src/project-invalid.data"),
            "FORBIDDEN_TOKEN\n",
        );
        write_temp_file(&fixtures.join("src/project-valid.data"), "clean\n");
        write_temp_file(&fixtures.join("expectations.json"), &serde_json::json!({
            "version":1,
            "cases":{
                "invalid":{"file":"rules/baseline/fixtures/src/project-invalid.data","expected":[{"id":"proj-selftest","engine":"regex","line":1}]},
                "valid":{"file":"rules/baseline/fixtures/src/project-valid.data","expected":[]}
            },
            "rules":{"proj-selftest":{"positive":"invalid","negative":"valid"}}
        }).to_string());
        ...
        ResolvedConfig {
            ...
            ast_enabled: true,
            ast_binary: None,
            project_rule_ids: resolved.project_rule_ids,
            project_ast_dirs: Default::default(),
    ...
    const PROJ_SELFTEST_JSON: &str = r#"{"proj":[{"id":"proj-selftest","severity":"high","pattern":"FORBIDDEN_TOKEN","resolution":"remove it","canary":"contains FORBIDDEN_TOKEN here","negativeCanary":["clean"]}]}"#;
    ...
    fn project_pack_broken_canary_fails_self_test() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let proj_json = r#"{"proj":[{"id":"proj-selftest","severity":"high","pattern":"FORBIDDEN_TOKEN","resolution":"remove it","canary":"this is clean text","negativeCanary":["clean"]}]}"#;
