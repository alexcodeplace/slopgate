...
fn run_actionlint_json(
    ...
    if stdout.is_empty() {
        return if res.status == Some(0) && res.stderr.trim().is_empty() {
            Ok(vec![])
        } else {
            Err("actionlint failed without a parseable diagnostic".into())
        };
    }
    let data: Value =
        serde_json::from_str(stdout).map_err(|e| format!("actionlint JSON parse error: {e}"))?;
    let items = data.as_array().ok_or("actionlint JSON must be an array")?;
    if items.iter().any(|item| {
        ...
        return Err("actionlint JSON contains an invalid diagnostic".into());
    }
    if res.status == Some(1) && items.is_empty() {
        return Err("actionlint nonzero exit without findings".into());
    }
