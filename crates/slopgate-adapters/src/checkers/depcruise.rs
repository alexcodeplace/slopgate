        ...
        .is_some_and(|violations| {
            violations.iter().all(|violation| {
                violation
                    ...
                    && violation.get("to").and_then(Value::as_str).is_some()
                    && violation
                        .get("rule")
                    ...
                    && violation
                        ...
                        .is_some_and(|severity| {
                            matches!(severity, "error" | "warn" | "info" | "ignore")
                        })
            }) && (status == Some(0) || !violations.is_empty())
        });
    if !valid {
        return CheckerRunResult {
            errors: vec!["dependency-cruiser: invalid summary/violation report".into()],
            ..Default::default()
        };
    }
    CheckerRunResult {
