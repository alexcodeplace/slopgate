...
pub fn run(
    ...
    let valid_files = data
        .get("files")
        .and_then(Value::as_array)
        .is_some_and(|items| {
            items
                .iter()
                .all(|item| item.as_str().is_some_and(|file| !file.is_empty()))
        });
    let valid_issues = data
        .get("issues")
        .and_then(Value::as_array)
        .is_some_and(|issues| {
            issues.iter().all(|issue| {
                issue
                    ...
                    && ISSUE_TYPES.iter().all(|kind| {
                        issue.get(*kind).is_none_or(|items| {
                            items
                                .as_array()
    ...
    if status != Some(0) || !valid_files || !valid_issues {
        return CheckerRunResult {
            errors: vec!["knip: invalid report or nonzero exit with --no-exit-code".into()],
            ..Default::default()
        };
    }
