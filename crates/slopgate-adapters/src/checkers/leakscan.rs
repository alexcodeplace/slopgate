...
use slopgate_core::severity::map_passthrough;
...
pub fn run(
    ...
    let records = data.get("violations").and_then(Value::as_array);
    if records.is_none()
        || data.get("errors").is_none_or(|value| {
            value
                .as_array()
                .is_none_or(|items| items.iter().any(|item| !item.is_string()))
        })
        || records.is_some_and(|items| {
            items.iter().any(|item| {
                item.get("file")
        ...
        || (status != Some(0) && records.is_some_and(Vec::is_empty))
    {
        return CheckerRunResult {
            errors: vec!["leakscan: invalid report or contradictory exit".into()],
            ..Default::default()
        };
    }
