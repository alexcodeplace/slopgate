...
use crate::gate::snapshot_violations_checked;
    ...
    let outer: Result<(), String> = {
        ...
        // Ratchet progress + burn-down
        {
            ...
            if bl.missing {
                ...
                match snapshot_violations_checked(config) {
                    Ok(current) => sections.push(AuditSection {
                        title: "Ratchet progress + burn-down".into(),
                        lines: ratchet_lines(bl.entries.len(), current.len(), &burndown),
                    }),
                    Err(errors) => notices.push(format!(
                        "ratchet progress skipped (required scanner failed: {})",
                        errors.join("; ")
                    )),
                }
            }
