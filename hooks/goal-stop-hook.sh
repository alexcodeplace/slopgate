#!/usr/bin/env bash
# Slopgate Stop hook — optional deterministic goal evidence gate.
...
run_slopgate() {
  if command -v timeout >/dev/null 2>&1; then
    timeout 5 node "$HERE/../bin/slopgate" goal-check --config "$CONFIG"
  else
    node "$HERE/../bin/slopgate" goal-check --config "$CONFIG"
  fi
}
