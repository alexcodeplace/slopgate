...
fn flags_every_leak_in_a_component() {
    ...
    assert!(r.contains(&"banned-import-in-component".into()), "got {r:?}");
    assert!(r.contains(&"raw-global-io-in-component".into()), "got {r:?}");
...
fn service_layer_is_exempt_even_with_fetch() {
    ...
    assert_eq!(report["scanned"].as_u64().unwrap(), 0, "service file should not be scanned");
    assert_eq!(report["violations"].as_array().unwrap().len(), 0);
}
...
fn locally_bound_fetch_is_not_the_global() {
    ...
    assert_eq!(report["violations"].as_array().unwrap().len(), 0, "shadowed fetch must not flag");
}
